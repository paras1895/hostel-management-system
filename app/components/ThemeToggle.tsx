...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    const initial = (saved || attr || lightTheme) as ThemeType;
    setTheme(initial);
  }, []);

  const toggleTheme = () => {
    const next = theme === darkTheme ? lightTheme : darkTheme;
    // write to DOM + storage
  ...
  return (
    <button
      ...
      className={`relative flex items-center justify-between w-16 h-8 rounded-full transition-colors duration-300 shadow-inner 
        ${
          theme === darkTheme
            ? "bg-neutral-800"
    ...
    >
      ...
      <svg
        ...
        className={`absolute left-2 w-4 h-4 transition-opacity duration-300 ${
          theme === darkTheme ? "opacity-0" : "opacity-100 text-[#132440]"
        }`}
      >
      ...
      <svg
        ...
        className={`absolute right-2 w-4 h-4 transition-opacity duration-300 ${
          theme === darkTheme ? "opacity-100 text-slate-600" : "opacity-0"
        }`}
      >
