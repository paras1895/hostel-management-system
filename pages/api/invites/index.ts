...
export default async function handler(
  ...
  if (!toStudentId)
    return res.status(400).json({ error: "toStudentId required" });
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (myRoom.studentCount >= myRoom.capacity)
        throw new Error("Your room is already full.");

      const targetRows = await tx.$queryRaw<
        { id: number; roomId: number | null; gender: string; year: string }[]
      >`
      ...
      if (target.roomId === myRoom.id)
        throw new Error("This student is already in your room.");

      if (
        myRoom.groupGender &&
        target.gender &&
        target.gender !== myRoom.groupGender
      ) {
        throw new Error(
          "Only same-gender roommates can be invited to this room."
        );
      }
